...
void frame_buffer_init() {
    ...
    unsigned int * FB_addr = (unsigned int *)
            Phy_To_Virt(boot_param->graphics_info.frameBufferBase);
    unsigned long * tmp;
    ...
    unsigned long * tmp1;
    for (int i = 0; i < pos.FB_length; i += PAGE_2M_SIZE) {
        tmp1 = Phy_To_Virt((unsigned long *) (*tmp & (~0xfffUL)) +
               (((unsigned long) ((unsigned long) FB_addr + i)
               >> PAGE_2M_SHIFT) & 0x1ff));
        unsigned long phy = boot_param->graphics_info.frameBufferBase + i;
        set_pdt(tmp1, mk_pdt(phy, PAGE_USER_Page | PAGE_PWT | PAGE_PCD));
    }

    pos.FB_addr = (unsigned int *) Phy_To_Virt(
            boot_param->graphics_info.frameBufferBase);
